...
function M_RegistrasiAnggotaDetail()
{
	...
	this.form_telepon = new Ext.form.TextField({
			fieldLabel		: 'Telepon'
		,	allowBlank		: true
		,	maxLength		: 50
...
function M_RegistrasiAnggotaList()
{
	...
	this.form_search_nm_p =  new Ext.form.TextField({
	...
	{	Ext.MessageBox.alert("",this.store.getTotalCount());
		var data = this.sm.getSelected();
	...
	this.do_search = function (){
		...
		this.store.setBaseParam('nm_p', this.form_search_nm_p.getValue());
		...
		this.store.load({
				...
				,	params	: {
					...
					,	limit		: this.pageSize
					,	load_type	: load_type
					,	nm_p		: this.form_search_nm_p.getValue()
